...
string Workspace::get_title() {

    return title;

}

void Workspace::set_title(const char *title) {

    this->title = title;

}

size_t Workspace::get_width() {

    return w;

}
size_t Workspace::get_height() {
...
void Workspace::set_bmp_output_buffer(char *buffer) {
    ...
    if (buffer) this->bmp_output_buffer = buffer;
...
void Workspace::set_hdr_output_buffer(Pixel3 *buffer) {
    ...
    if (buffer) this->hdr_output_buffer = buffer;
...
int Workspace::dump_mpg(const char *path) {
...
int Workspace::load_lmw(const char *file_path) {
    ...
    json_t *file_obj; const char* file_name;
    ...
    json_t *layer_obj; Layer *layer; const char *layer_name;
    ...
    for (size_t i = 0; i < json_array_size(layer_arr); ++i) {
        ...
        if (active) { layer->enable(); } else { layer->disable(); }
    ...
    if (animation_obj) {
        ...
        Layer *layer; const char *layer_name;
...
int Workspace::load_exr(const char *file_path) {
    ...
    // read layers
    string layer_name; Pixel3* pixels;
    if (names.size()) {
        ...
        for (set<string>::iterator i = names.begin(); i != names.end(); ++i)
        {
            ...
            if (!pixels) lmerr("Failed to allocate memory for new layer");
...
int Workspace::save_lmw(const char *file_name) {
    ...
    if (animation) {
        ...
        for (set<Timeline*>::iterator it = timelines.begin(); it != timelines.end(); ++it) {
            ...
            for(vector<Keyframe>::iterator f = frames.begin(); f != frames.end(); ++f) {
