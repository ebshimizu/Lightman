...
vector<Keyframe> Timeline::get_keyframes() {
...
Keyframe Timeline::get_frame(float t) {
    ...
    if (keyframes.size() == 0) { return Keyframe(0, vec3(1,1,1)); }
    if (keyframes.size() == 1) { return keyframes.front(); }
    ...
    if (t < t_min) { return keyframes.front(); }
    if (t > t_max) { return keyframes.back();  }
    ...
    vec3 mod = prev_r * prev_key->get_modulator() +
               next_r * next_key->get_modulator();
...
float Animation::get_len() {

    return len;

}

void Animation::set_len(size_t len) {

    this->len = len;

}

size_t Animation::get_fps() {

    return fps;

}

void Animation::set_fps(size_t fps) {

    this->fps = fps;

}

void Animation::add_timeline(Timeline *timeline) {

    timelines.insert(timeline);

}

void Animation::del_timeline(Timeline *timeline) {

    timelines.erase(timeline);

}

set<Timeline*> Animation::get_timelines() {
