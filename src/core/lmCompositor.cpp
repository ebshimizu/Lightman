...
Compositor::~Compositor() {
    ...
    if (compose_buffer) delete[] compose_buffer;

    // free all layers
    layers.clear();
}

size_t Compositor::get_width() {

    return w;
}

size_t Compositor::get_height() {

    return h;

}

set<Layer*> Compositor::get_layers() {
...
void Compositor::del_layer(Layer *layer) {
...
void Compositor::set_output_buffer(Pixel3 *buffer) {
    ...
    if (buffer) output_buffer = buffer;
...
void Compositor::render() {
    ...
    // get target buffer
    Pixel3 * __restrict__ target = output_buffer ?
                                   output_buffer :
                                   compose_buffer;
